...
function errorShake() {
    $(".game-container")
      .animate({left: '+=1em'}, "normal")
      .animate({left: '-=2em'}, "normal")
      .animate({left: '+=2em'}, "normal")
      .animate({left: '-=1em'}, "normal", function() {
        // Reset the left position of the element
        $(this).css({ left: 0 });
      });
  }
...
function testWord(num_in) {
    ...
    if (num_in == ansr) {
        $(".game-container").after("<div class=\"win popup\">You Won!<div class=\"word-reveal\">press enter to play again</div></div>");
        playing = false;
        return
    } 
    if (rowNum == rows-1) {
        playing = false;
        $(".game-container").after("<div class=\"lose popup\">You lost!<div class=\"word-reveal\">The number was: " + ansr + "</div><div class=\"word-reveal\">press enter to play again</div></div>");
        return;
    }
